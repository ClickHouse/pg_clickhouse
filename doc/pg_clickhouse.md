...
*   `Tuple` and `Map` columns import as `text[]` and `text[][]` with a
    `NOTICE`. PostgreSQL's `record` pseudotype cannot define a table column, so
    a `Tuple` fills an array with its fields, while a `Map` fills a nested array
