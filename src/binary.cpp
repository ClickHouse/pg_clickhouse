...
#include "minicoro.h"
	...
	{
		ch_binary_streaming_state *st = (ch_binary_streaming_state *)mco_get_user_data(co);

		try
		{
			st->client->Select(clickhouse::Query(st->sql)
	...
	ch_binary_begin_streaming(ch_binary_connection_t *conn, const ch_query *query, bool (*check_cancel)(void))
	{
		mco_desc   desc;
		mco_result res;
	...
	ch_binary_streaming_read_row(ch_binary_streaming_state *st)
	{
		...
		try
		{
			...
			{
				st->values[i] = make_datum(block[i], st->current_row, &st->coltypes[i], &st->nulls[i]);
			}
		}
		catch (const std::exception &e)
		{
	...
	Datum
	ch_binary_streaming_value(ch_binary_streaming_state *st, size_t col, Oid *valtype, bool *is_null)
	{
