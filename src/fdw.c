...
clickhouseIterateForeignScan(ForeignScanState * node)
{
	...
	if (fsstate->ch_cursor == NULL)
	{
		...
		fsstate->is_streaming = fsstate->fetch_size > 0
			&& fsstate->conn.methods->streaming_query != NULL;

		if (fsstate->is_streaming)
		{
			fsstate->ch_cursor =
				fsstate->conn.methods->streaming_query(
													   fsstate->conn.conn, &query,
													   fsstate->fetch_size);
		}
		else
		{
			...
			fsstate->ch_cursor =
				fsstate->conn.methods->simple_query(
													fsstate->conn.conn, &query);
		}
...
foreign_window_ok(PlannerInfo * root, RelOptInfo * window_rel)
{
	...
	fpinfo->relation_name = makeStringInfo();
	appendStringInfo(fpinfo->relation_name, "Window on (%s)",
					 ofpinfo->relation_name->data);
...
#if (PG_VERSION_NUM < 120000)
	window_path = create_foreignscan_path(root,
										  window_rel,
										  ...
										  NIL); /* no fdw_private */
#else
	window_path = create_foreign_upper_path(root,
											window_rel,
											root->upper_targets[UPPERREL_WINDOW],
											rows,
#if PG_VERSION_NUM >= 180000
						  ...
						  RelOptInfo * ordered_rel)
{
	...
	foreach(lc, root->sort_pathkeys)
	{
		...
		sort_expr = chfdw_find_em_expr_for_input_target(root,
														pathkey_ec,
														input_rel->reltarget->exprs != NIL
														? input_rel->reltarget
														: root->upper_targets[ifpinfo->stage]);

		if (sort_expr == NULL ||
			!chfdw_is_foreign_expr(root, input_rel, sort_expr))
			return;
	}
