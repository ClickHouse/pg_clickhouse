...
/*
 * Query to generate table for doc/pg_clickhouse.md. Keep in sync with the
 * column descriptors pgch_pg_type_for() reports. On change, re-run and paste
 * the output into doc/pg_clickhouse.md.
 * Perl: https://stackoverflow.com/a/58443028/79202
...
/* Return the aggregate-state wrapper `declaration` names, NULL for other types */
static const char*
 ...
 * literal parameters, the aggregate name, then one type per aggregate argument
 * Report the name through `out_func` and return the first argument type, NULL
 * for a state that names none
 */
static char*
take_aggregate(char* params, char** out_func) {
    ...
    *out_func = NULL;
    while ((param = take_parameter(&params)) != NULL) {
        ...
        /* Literals ahead of the name parameterize the aggregate */
...
/*
 * Return the PostgreSQL type declaration importing a ClickHouse column type
...
parse_type(
     ...
     * Tuple, which neither the reader nor the INSERT writer takes as jsonb
     */
    ...
    *is_nullable = type.nullable;
    ...
    /* Tuple and Map reach the pseudotypes no table column can hold */
    if (!pgch_pg_type_is_column(type)) {
        elog(
            NOTICE,
            "pg_clickhouse: ClickHouse <%.*s> type was translated to <TEXT> type for "
        ...
        decl = type.ndims > 0 ? "TEXT[]" : "TEXT";
    } else {
        decl = format_type_with_typemod(type.typid, type.typmod);
    }

    /*
     * One PostgreSQL array type spans every depth, so the declaration carries
     * the rest as extra dimensions, which attndims keeps for the INSERT writer
     */
